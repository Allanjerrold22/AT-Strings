...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    client.fetch(query)
      .then(data => {
        data.map((ele) => {
          ele.User.filter((curr) => curr.uid.includes("cart") && curr.uid.includes(auth.currentUser.uid)).forEach((item, inx) => {
            lis = [...lis, item.list]
          })
        })
        _setProducts(lis)
  ...
  return (
    <div style={{}}>
      ...
      <div className="cart-product-container" style={{ marginLeft: 8, marginRight: 8, marginTop: 16 }}>
        ...
        {_products.map((ele, inx) => {
          return (
            <div className="card-product" style={{ background: "#F5F5F5", display: 'flex', flexDirection: 'row', marginTop: 12 }}>
              <img src={urlFor(ele[0].cover.options.source)} style={{ width: 100, height: 100, objectFit: 'cover', borderRadius: 8 }} />
              <div className="cart-text-container" style={{ marginLeft: 12, marginTop: 6, width: '100%' }}>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 500, color: '#252525' }}>{ele[0].name}</p>
                <p style={{ margin: 0, fontSize: 12, fontWeight: 400, color: '#656565' }}>Size: {ele[0].size}</p>
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                  ...
                  <p style={{ margin: 0, fontSize: 14, fontWeight: 400, color: 'green' }}>${ele[0].price}</p>
                </div>
              </div>
      ...
      <div className="order-summary" style={{ position: 'fixed', bottom: 0, height: 200, width: '100%', background: '#ffff', borderTopRightRadius: 10, borderTopLeftRadius: 10 }}>
        <div>
          <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%', alignItems: 'center', marginTop: 12 }}>
            ...
            <p style={{ margin: 0, fontSize: 14, fontWeight: 400, color: '#656565', marginRight: 12 }}>{_products.length} Product(s)</p>
          </div>
